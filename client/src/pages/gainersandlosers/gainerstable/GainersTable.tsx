...
import s from './GainersTable.module.scss';
...
const GainersTable = ({ gainersArr, watchListData }: { gainersArr: Coin[]; watchListData: Coin[] }) => {
  const dispatch = useDispatch();
  const [sortedCryptosList, setSortedCryptosList] = useState(gainersArr || []);
  ...
  const fieldSort = (field: any) => {
    ...
    const sortedList = gainersArr ? [...gainersArr] : [];
    ...
    <div className={s.gainers_table}>
      <div className={s.tableContainer}>
        ...
        <table className={s.table}>
          ...
          <tbody>
            {Array.isArray(gainersArr) &&
              (sortedCryptosList || gainersArr).map((coin: any) =>
                !coin ? null : (
                  <tr key={coin?.id}>
